"""
ATM-Net++ Optimized Training v2
Implements all 6 expert-recommended fixes:
  1. Weighted sampler (10x weight for rare upper classes)
  2. Per-class inverse-frequency Dice weights (30x for IVD-8)
  3. 256×256 resolution (critical for small structures)
  4. All available foreground slices (not capped at 20)
  5. Boundary-aware loss term
  6. Reduced label smoothing (0.03 not 0.05)
  
Expected: Val Dice ~0.75-0.80 (up from 0.615)
Hardware: RTX 3050 4GB, fp32, BS=2, accum=4
"""
...
# FIX 3: Increase resolution to 256×256
IMG_SIZE    = 256
NUM_CLASSES = 19
BATCH_SIZE  = 2      # fp32 on 4GB VRAM
...
RARE_CLASSES = [7, 8, 16, 17]  # Vert-7/8, IVD-7/8 — highest imbalance

# FIX 2: Per-class inverse-frequency weights (rare structures get much higher weight)
CLASS_WEIGHTS = torch.tensor([
    0.0,                                    # background — not used in Dice
    1.0, 1.0, 1.0, 1.5, 2.0, 3.0, 6.0, 12.0,  # Vert 1-8 (upper = rarer)
    1.0,                                    # Sacrum
    6.0, 4.0, 4.0, 5.0, 6.0, 9.0, 14.0, 30.0,  # IVD 1-8 (IVD-8 = 30x)
    0.0,                                    # Canal — absent in data
]).float()
...
# ── PRE-PROCESS TO NPZ ────────────────────────────────────────────────
def preprocess_to_cache(pids, split, max_spp=9999):
    """
    Save slices to individual .npy files in a cache folder.
    No RAM limit — loads lazily per batch.
    """
    split_dir = CACHE_DIR / f"{split}_t2_{IMG_SIZE}"
    index_file = split_dir / "index.json"

    if index_file.exists():
        print(f"  {split}: loading index from cache ({split_dir.name})...")
        idx = json.load(open(index_file))
        return idx["files"], idx["rare_flags"]

    split_dir.mkdir(parents=True, exist_ok=True)
    print(f"  {split}: preprocessing {len(pids)} patients (ALL fg slices)...")
    files, rare_flags = [], []
    t0 = time.time()
    ...
    for pid in pids:
        ...
        for s in range(lo, hi):
            rm = remap(mv[s])
            if fg_ratio(rm) < 0.003: continue
            p1, p99 = np.percentile(iv[s], [0.5, 99.5])
            img_n = np.clip((iv[s]-p1)/(p99-p1+1e-8), 0, 1).astype(np.float16)
            img_r = cv2.resize(img_n.astype(np.float32),(IMG_SIZE,IMG_SIZE),
                               interpolation=cv2.INTER_LINEAR).astype(np.float16)
            msk_r = cv2.resize(rm.astype(np.float32),(IMG_SIZE,IMG_SIZE),
                               interpolation=cv2.INTER_NEAREST).astype(np.uint8)
            # Save as individual file
            fname = f"{pid}_s{s:04d}.npz"
            fpath = split_dir / fname
            np.savez_compressed(fpath, img=np.clip(img_r,0,1),
                                msk=np.clip(msk_r,0,NUM_CLASSES-1))
            files.append(str(fpath))
            rare_flags.append(1.0 if rare_ratio(rm) > 0.0005 else 0.1)

    # Save index
    idx = {"files": files, "rare_flags": rare_flags}
    json.dump(idx, open(index_file,"w"))
    rare_count = sum(1 for r in rare_flags if r > 0.5)
    print(f"  {split}: {len(pids)} patients → {len(files)} slices "
          f"({rare_count} rare) | {time.time()-t0:.0f}s | cached to {split_dir.name}/")
    return files, rare_flags
...
class Aug:
    def __call__(self, img, msk):
        ...
        if random.random()<0.3:
            ...
            hs,ws=random.randint(10,36),random.randint(10,36)
            img[max(0,cy-hs//2):min(IMG_SIZE,cy+hs//2),
                max(0,cx-ws//2):min(IMG_SIZE,cx+ws//2)]=0
        return img.astype(np.float32), msk.astype(np.int64)

class NPZDataset(Dataset):
    """Pre-loads epoch subset into RAM. Zero disk I/O during training."""
    def __init__(self, files, aug=None):
        self.files = files; self.aug = aug
        self.imgs  = None;  self.msks = None

    def preload(self, indices=None):
        """Load selected indices into RAM."""
        flist = [self.files[i] for i in indices] if indices else self.files
        imgs, msks = [], []
        for f in flist:
            d = np.load(f)
            imgs.append(d["img"].astype(np.float32))
            msks.append(d["msk"].astype(np.int64))
        self.imgs = np.stack(imgs)
        self.msks = np.stack(msks)
        self.n    = len(self.imgs)

    def __len__(self):
        return self.n if self.imgs is not None else len(self.files)

    def __getitem__(self, i):
        if self.imgs is not None:
            img = self.imgs[i]; msk = self.msks[i].copy()
        else:
            d = np.load(self.files[i])
            img = d["img"].astype(np.float32); msk = d["msk"].astype(np.int64)
        if self.aug: img, msk = self.aug(img.copy(), msk)
        return torch.from_numpy(img[None]).float(), torch.from_numpy(msk).long()

# ── MODEL (same ResUNet, slightly less dropout for 256×256) ───────────
class CA(nn.Module):
...
class ResUNet(nn.Module):
    def __init__(self,b=32,nc=NUM_CLASSES,drop=0.25):  # Reduced dropout for 256×256
        super().__init__()
        ...
        # FIX 5: Aux head at full resolution for rare small structures
        self.aux_head=nn.Sequential(nn.Conv2d(b,b,3,1,1,bias=False),nn.BatchNorm2d(b),nn.ReLU(True),nn.Conv2d(b,nc,1))
    def forward(self,x):
        sz=x.shape[2:]
        ...
        o1=self.out(d)
        aux=self.aux_head(d)  # full-resolution auxiliary output
        return (o1,o2,o3,aux) if self.training else o1

# ── LOSSES ────────────────────────────────────────────────────────────
def dice_loss_weighted(logits, tgt, weights, sm=1e-6):
    """FIX 2: Weighted Dice — rare classes get 30x weight."""
    ...
    w=weights[1:].to(logits.device).view(1,C-1)
    denom=(mask*w).sum().clamp(min=1)
    return 1.0-(dice*mask*w).sum()/denom

def boundary_loss(logits, tgt, sm=1e-6):
    """FIX 5: Penalize predictions near class boundaries."""
    pred_soft=F.softmax(logits,1)
    oh=F.one_hot(tgt.clamp(0,NUM_CLASSES-1),NUM_CLASSES).permute(0,3,1,2).float()
    # Approximate boundary via max-pool dilation minus original
    pooled=F.max_pool2d(oh[:,1:],3,stride=1,padding=1)
    boundary=(pooled-oh[:,1:]).clamp(0,1)
    # Penalize missing boundary predictions, weighted by class rarity
    w=CLASS_WEIGHTS[1:].to(logits.device).view(1,-1,1,1)
    loss=(boundary*(1-pred_soft[:,1:])*w).sum()/(boundary*w+1e-8).sum().clamp(min=1)
    return loss
...
def compound(logits, tgt):
    ...
    ce =F.cross_entropy(logits,tc,label_smoothing=0.03)  # FIX 6: Reduced smoothing
    dl =dice_loss_weighted(logits,tc,CLASS_WEIGHTS)       # FIX 2: Weighted
    fl =focal_loss(logits,tc)
    bl =boundary_loss(logits,tc)                          # FIX 5: Boundary
    return ce + dl + 0.3*fl + 0.2*bl

def total_loss(outs, tgt):
    o1,o2,o3,aux=outs
    main=compound(o1,tgt)+0.3*compound(o2,tgt)+0.15*compound(o3,tgt)
    # FIX 5: Auxiliary head with 5x weight on rare classes
    tc=tgt.clamp(0,NUM_CLASSES-1)
    rare_mask=sum((tc==c).float() for c in RARE_CLASSES).clamp(0,1)
    aux_ce=F.cross_entropy(aux,tc,reduction='none')
    aux_loss=(aux_ce*(1+4*rare_mask)).mean()  # 5x on rare class pixels
    return main + 0.4*aux_loss
...
def main():
    print("="*68)
    print("  ATM-Net++ Optimized Training v2")
    print("  Fixes: weighted sampler + class weights + 256px + all slices + boundary loss")
    print("="*68)
    ...
    print(f"  Config : {EPOCHS} epochs  BS={BATCH_SIZE}x{ACCUM_STEPS}={BATCH_SIZE*ACCUM_STEPS}eff"
          f"  LR={LR}  {IMG_SIZE}x{IMG_SIZE}  ALL slices\n")

    print("  Preprocessing to cache...")
    tr_files, tr_rare = preprocess_to_cache(tr_pids, "train_v2")
    va_files, va_rare = preprocess_to_cache(va_pids, "val_v2")
    print(f"  Train slices: {len(tr_files)} | Val slices: {len(va_files)}\n")

    aug = Aug()
    tr_ds = NPZDataset(tr_files, aug)
    va_ds = NPZDataset(va_files)

    # Pre-load val into RAM (use up to 2000 slices to fit in 7GB RAM)
    va_sel = list(range(min(2000, len(va_files))))
    print(f"  Pre-loading {len(va_sel)} val slices into RAM...")
    va_ds.preload(va_sel)
    va_dl = DataLoader(va_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    rare_train = sum(1 for r in tr_rare if r > 0.5)
    print(f"  Rare slices in pool: {rare_train}/{len(tr_rare)}")
    print(f"  Per-epoch: preload 5000 slices (all rare + random normal) into RAM\n")

    model = ResUNet(b=32, nc=NUM_CLASSES, drop=0.25).to(device)
    n_p   = sum(p.numel() for p in model.parameters())
    print(f"  Model  : ResUNet+CBAM+Dropout+AuxHead  {n_p/1e6:.2f}M params")
    print(f"  Val batches: {len(va_dl)} | Train: epoch-dynamic (~2500/epoch)")
    ...
    if CKPT_BEST.exists():
        ...
        if missing: print(f"  New layers (aux_head): {len(missing)} keys — initialized from scratch")
    ...
    for ep in range(start_epoch, EPOCHS+1):
        ...
        # Re-sample 5000 slices each epoch, preload into RAM
        if len(tr_files) > 5000:
            rare_idx  = [i for i,r in enumerate(tr_rare) if r > 0.5]
            normal_idx= [i for i,r in enumerate(tr_rare) if r <= 0.5]
            n_normal  = min(5000 - len(rare_idx), len(normal_idx))
            selected  = rare_idx + random.sample(normal_idx, n_normal)
            rare_ep   = [tr_rare[i] for i in selected]
        else:
            selected  = list(range(len(tr_files)))
            rare_ep   = tr_rare

        # Preload epoch data into RAM (~1.3GB for 5000 × 256×256 fp32)
        tr_ds.preload(selected)
        sampler_ep = WeightedRandomSampler(torch.tensor(rare_ep), len(rare_ep), replacement=True)
        tr_dl = DataLoader(tr_ds, batch_size=BATCH_SIZE, sampler=sampler_ep, num_workers=0)
    ...
    print(f"  Epoch: {ckpt['epoch']} | best_dice={ckpt['best_dice']:.4f}\n")
    ...
    with torch.no_grad():
        for imgs,msks in va_dl:
            ...
            pred_np=probs.argmax(1).cpu().numpy(); gt_np=msks.cpu().numpy()
            for b in range(pred_np.shape[0]):
                n_sl+=1
                for c in range(1,NUM_CLASSES):
                    p=(pred_np[b]==c).astype(float).ravel(); t=(gt_np[b]==c).astype(float).ravel()
                    if t.sum()==0 and p.sum()==0: continue
                    tp=(p*t).sum(); fp=(p*(1-t)).sum(); fn=((1-p)*t).sum()
