"""
ATM-Net++ GPU Training — Resumes from epoch 12 checkpoint
RTX 3050 4GB VRAM | 172×256×256 images | Mixed precision (AMP)

Improvements over CPU run:
  - GPU (RTX 3050) ~20-30x faster per epoch: 10min → ~20sec
  - 256×256 resolution (7× more pixels than 96×96)
  - AMP (fp16) — doubles effective VRAM, faster compute
  - Larger batch size (16 at 256×256 with fp16)
  - Resumes from best CPU checkpoint (epoch 11, Dice=0.4425)
  - Expected: Dice 0.80+ in ~2h, Dice 0.90+ in ~5-8h
"""
...
# Resume from whichever checkpoint is newer / has higher dice
_GPU_CKPT  = Path(r"c:\project\Spine Segmentation\ATM-Net++\outputs\gpu_run\best_model.pth")
_CPU_CKPT  = Path(r"c:\project\Spine Segmentation\ATM-Net++\outputs\high_perf_run\best_model.pth")
CPU_CKPT   = _GPU_CKPT if _GPU_CKPT.exists() else _CPU_CKPT
OUT_DIR.mkdir(parents=True, exist_ok=True)

IMG_SIZE    = 192       # 192x192 stable on RTX 3050 4GB
NUM_CLASSES = 19
BATCH_SIZE  = 2         # Reduced: prevents VRAM fragmentation crashes on Windows
EPOCHS      = 150
LR          = 5e-4
WEIGHT_DECAY= 1e-4
MAX_SPP     = 16
...
def fg_score(m):
    rm = remap(m)
    return float((rm > 0).sum()) / max(rm.size, 1) + len(np.unique(rm[rm > 0])) * 0.01
...
def load_dataset(pids, label):
    ...
    for pid in pids:
        ...
        ranked = sorted(range(lo, hi), key=lambda s: fg_score(mv[s]), reverse=True)[:MAX_SPP]
        for s in ranked:
    ...
    def __call__(self, img, msk):
        ...
        if random.random() < 0.3:
            try:
                ...
                h, w = img.shape
                dx = gaussian_filter(np.random.randn(h,w), self.sz*0.06) * (self.sz*0.6)
                dy = gaussian_filter(np.random.randn(h,w), self.sz*0.06) * (self.sz*0.6)
...
# ── MODEL: ResUNet + CBAM (same architecture as CPU run) ─────────────
...
class ResBlock(nn.Module):
    def __init__(self, ch):
        ...
        self.ca=ChannelAttn(ch); self.sa=SpatialAttn(); self.act=nn.ReLU(True)
...
# ── LOSSES (fully vectorized — no Python loops on GPU tensors) ────────
def dice_loss(logits, tgt, sm=1e-6):
    """Soft Dice loss — fully vectorized, no per-class Python loops."""
    B, C, H, W = logits.shape
    soft = F.softmax(logits, 1)  # (B,C,H,W)
    oh   = F.one_hot(tgt.clamp(0, C-1), C).permute(0,3,1,2).float()  # (B,C,H,W)
    # Only foreground classes 1..C-1
    p = soft[:, 1:].reshape(B, C-1, -1)   # (B, C-1, H*W)
    t = oh[:,  1:].reshape(B, C-1, -1)    # (B, C-1, H*W)
    inter = (p * t).sum(-1)               # (B, C-1)
    union = p.sum(-1) + t.sum(-1)         # (B, C-1)
    # Only count classes that exist in this batch
    mask  = (t.sum(-1) > 0).float()       # (B, C-1)
    dice  = (2*inter + sm) / (union + sm) # (B, C-1)
    n     = mask.sum().clamp(min=1)
    return 1.0 - (dice * mask).sum() / n

def tversky_loss(logits, tgt, alpha=0.3, beta=0.7, sm=1e-6):
    """Tversky loss — vectorized."""
    B, C, H, W = logits.shape
    soft = F.softmax(logits, 1)
    oh   = F.one_hot(tgt.clamp(0, C-1), C).permute(0,3,1,2).float()
    p    = soft[:, 1:].reshape(B, C-1, -1)
    t    = oh[:,  1:].reshape(B, C-1, -1)
    tp   = (p * t).sum(-1)
    fp   = (p * (1-t)).sum(-1)
    fn   = ((1-p) * t).sum(-1)
    mask = (t.sum(-1) > 0).float()
    tv   = (tp + sm) / (tp + alpha*fp + beta*fn + sm)
    n    = mask.sum().clamp(min=1)
    return 1.0 - (tv * mask).sum() / n
...
def compound(logits, tgt):
    tgt_c = tgt.clamp(0, NUM_CLASSES-1)
    return (F.cross_entropy(logits, tgt_c)
            + dice_loss(logits, tgt_c)
            + 0.5 * tversky_loss(logits, tgt_c)
            + 0.3 * focal_loss(logits, tgt_c))
...
def full_metrics(logits, tgt):
    ...
    P=defaultdict(list); R=defaultdict(list); F1=defaultdict(list)
    for b in range(pred.shape[0]):
        for c in range(1,NUM_CLASSES):
            ...
            P[c].append(pr); R[c].append(re); F1[c].append(2*pr*re/(pr+re+sm))
...
def main():
    print("="*68)
    print("  ATM-Net++ GPU Training — RTX 3050 4GB — SPIDER Dataset")
    print("="*68)
    ...
    print(f"  Cfg  : {EPOCHS} epochs  BS={BATCH_SIZE}  LR={LR}  {IMG_SIZE}×{IMG_SIZE}  AMP=ON")
    print(f"  Slices: top-{MAX_SPP} fg-ranked/patient\n")
    ...
    # num_workers=0 avoids Windows paging file / multiprocessing CUDA DLL errors
    tr_dl = DataLoader(SliceDS(ti,tm,aug), batch_size=BATCH_SIZE, shuffle=True,
                       num_workers=0, pin_memory=True)
    va_dl = DataLoader(SliceDS(vi,vm),     batch_size=BATCH_SIZE, shuffle=False,
                       num_workers=0, pin_memory=True)

    # Build model
    model = ResUNet(b=32, nc=NUM_CLASSES).to(device)
    n_p   = sum(p.numel() for p in model.parameters())
    print(f"\n  Model : ResUNet+CBAM  {n_p/1e6:.2f}M params")
    print(f"  Batches: {len(tr_dl)} train | {len(va_dl)} val/epoch")

    # ── Load CPU checkpoint weights (transfer learning from CPU run) ──
    start_epoch = 1
    best        = 0.0
    hist        = []

    if CPU_CKPT.exists():
        print(f"\n  Loading checkpoint: {CPU_CKPT.name}")
        ckpt = torch.load(CPU_CKPT, map_location=device)
        missing, unexpected = model.load_state_dict(ckpt["model_state_dict"], strict=False)
        if missing: print(f"  Missing keys: {len(missing)}")
        best = ckpt.get("best_dice", 0.0)
        start_epoch = ckpt.get("epoch", 17) + 1
        print(f"  Resuming from epoch {start_epoch}  |  Best Dice so far: {best:.4f}")
        print(f"  Continuing GPU training → target Dice >0.90")
    else:
        print("  No CPU checkpoint found. Starting fresh.")

    optim  = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    sched  = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optim, T_0=30, T_mult=2, eta_min=1e-6)
    scaler = GradScaler(enabled=False)  # Disable AMP — causes CUDA crashes on RTX 3050 + Windows
    ...
    print(f"\n  {'Ep':>4}  {'TrLoss':>8}  {'TrDice':>8}  {'VaLoss':>8}  "
          f"{'VaDice':>8}  {'Best':>8}  {'LR':>8}  {'Sec':>5}")
    print("  "+"─"*69)

    for ep in range(start_epoch, EPOCHS+1):
        torch.cuda.empty_cache()   # clear VRAM fragmentation each epoch
        ...
        for imgs,msks in tr_dl:
            ...
            try:
                with autocast(enabled=False):   # AMP disabled for stability
                    outs = model(imgs)
                ...
                if "out of memory" in str(e).lower() or "CUDA" in str(e):
                    torch.cuda.empty_cache(); optim.zero_grad(set_to_none=True); continue
                raise
        ...
        print(f"  {ep:>4}  {tl:>8.4f}  {td:>8.4f}  {vl:>8.4f}  "
              f"{vd:>8.4f}  {best:>8.4f}  {lr_n:.2e}  {ep_s:>4.0f}s{flag}")
        hist.append({"ep":ep,"tl":tl,"td":td,"vl":vl,"vd":vd,"lr":lr_n})
    ...
    print("  "+"─"*69)
    print(f"\n  Training done in {ttot/60:.1f} min ({ttot/3600:.2f}h)  |  Best Dice: {best:.4f}")
    with open(OUT_DIR/"history.json","w") as f: json.dump(hist,f,indent=2)

    # ── FINAL EVALUATION WITH TTA ─────────────────────────────────────
    print("\n"+"="*68)
    print("  FINAL EVALUATION — Best Checkpoint + TTA (4×)")
    print("="*68)
    ckpt = torch.load(CKPT_BEST, map_location=device)
    model.load_state_dict(ckpt["model_state_dict"]); model.eval()
    print(f"  Checkpoint epoch: {ckpt['epoch']}  |  Training dice: {ckpt['best_dice']:.4f}\n")
    ...
    status=("🎯 TARGET ACHIEVED: Dice ≥ 0.90!" if md>=0.90 else
            "📈 Excellent! Very close to target" if md>=0.80 else
            "📊 Strong improvement — continue training" if md>=0.70 else
            "📉 Good progress — more epochs needed")

    print(f"""
  ╔══════════════════════════════════════════════════════════════════╗
  ║     EVALUATION REPORT — ATM-Net++ — SPIDER (GPU + TTA)         ║
  ╠══════════════════════════════════════════════════════════════════╣
  ║  GPU         : {torch.cuda.get_device_name(0):<48}║
  ║  Resolution  : {IMG_SIZE}×{IMG_SIZE}  |  AMP fp16  |  Epochs: {ep:<16}║
  ╠══════════════════════════════════════════════════════════════════╣
  ║  Vertebrae Dice  : {vdm:.4f}                                      ║
  ║  IVD Dice        : {idm:.4f}                                      ║
  ║  Mean Dice       : {md:.4f}  ← PRIMARY METRIC                    ║
  ║  Mean IoU        : {mi:.4f}                                      ║
...
""")
    ...
    final={"mean_dice":md,"mean_iou":mi,"mean_f1":mf1,
           ...
           "gpu":torch.cuda.get_device_name(0),
           "best_training_dice":float(best),"tta":True,
           "resolution":f"{IMG_SIZE}x{IMG_SIZE}","per_class":res}
