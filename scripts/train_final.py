"""
ATM-Net++ Final Training
- Pre-processes dataset to compressed NPZ files (run once, ~2min)
- Fast epoch training from NPZ (~3-4min/epoch)
- Resumes from epoch 71 (Dice=0.591)
- Fixes: stronger dropout, label smoothing, no LR restarts, more data
"""
...
NUM_CLASSES = 19
BATCH_SIZE  = 4
ACCUM_STEPS = 2       # effective BS = 4*2 = 8
EPOCHS      = 150
LR          = 8e-5
WEIGHT_DECAY= 1e-3
MAX_SPP     = 20      # slices per patient
PATIENCE    = 40
SEED        = 42
...
# ── PRE-PROCESS DATASET TO NPZ (run once) ────────────────────────────
def preprocess_to_cache(pids, split, modality="t2"):
    """Convert MHA files to fast-loading NPZ. Cached on disk."""
    cache_file = CACHE_DIR / f"{split}_{modality}_{IMG_SIZE}.npz"
    if cache_file.exists():
        print(f"  {split}: loading from cache {cache_file.name}...")
        d = np.load(cache_file)
        return d["imgs"], d["msks"]

    print(f"  {split}: preprocessing {len(pids)} patients...")
    imgs, msks = [], []
    t0 = time.time()
    for pid in pids:
        ...
        # Rank slices by foreground, pick top MAX_SPP
        fg = [(s, float((remap(mv[s])>0).sum()/mv[s].size)) for s in range(lo,hi)]
        fg = sorted(fg, key=lambda x:-x[1])
        fg = [s for s,r in fg if r>0.005][:MAX_SPP]
        for s in fg:
            p1,p99 = np.percentile(iv[s],[0.5,99.5])
            img_n = np.clip((iv[s]-p1)/(p99-p1+1e-8),0,1).astype(np.float16)  # fp16 saves RAM
            img_r = cv2.resize(img_n.astype(np.float32),(IMG_SIZE,IMG_SIZE),
                               interpolation=cv2.INTER_LINEAR).astype(np.float16)
            msk_r = cv2.resize(remap(mv[s]).astype(np.float32),(IMG_SIZE,IMG_SIZE),
                               interpolation=cv2.INTER_NEAREST).astype(np.uint8)
            imgs.append(img_r); msks.append(np.clip(msk_r,0,NUM_CLASSES-1))

    imgs_arr = np.stack(imgs)   # (N, H, W) fp16
    msks_arr = np.stack(msks)   # (N, H, W) uint8
    np.savez_compressed(cache_file, imgs=imgs_arr, msks=msks_arr)
    size_mb = cache_file.stat().st_size // 1024**2
    print(f"  {split}: {len(pids)} patients → {len(imgs)} slices "
          f"({time.time()-t0:.0f}s) | cached {size_mb}MB")
    return imgs_arr, msks_arr
...
class Aug:
    def __call__(self, img, msk):
        ...
        if random.random()<0.3:
            ...
            hs,ws=random.randint(8,28),random.randint(8,28)
...
class ResUNet(nn.Module):
    def __init__(self,b=32,nc=NUM_CLASSES,drop=0.35):
        super().__init__()
...
def compound(logits, tgt):
    ...
    ce = F.cross_entropy(logits, tc, label_smoothing=0.05)
    dl = dice_loss(logits, tc)
    fl_ce = F.cross_entropy(logits, tc, reduction='none')
    fl = ((1-torch.exp(-fl_ce))**2 * fl_ce).mean()
    return ce + dl + 0.3*fl
...
def main():
    print("="*65)
    print("  ATM-Net++ Final Training — NPZ Cache — Overfitting Fixes")
    print("="*65)
    ...
    print("  Preprocessing to NPZ cache (fast load, uses fp16+uint8)...")
    ti, tm = preprocess_to_cache(tr_pids, "train")
    vi, vm = preprocess_to_cache(va_pids, "val")
    print(f"  RAM used: ~{(ti.nbytes+tm.nbytes+vi.nbytes+vm.nbytes)//1024**2} MB\n")

    aug=Aug()
    tr_dl=DataLoader(NPZDataset(ti,tm,aug), batch_size=BATCH_SIZE, shuffle=True,  num_workers=0)
    va_dl=DataLoader(NPZDataset(vi,vm),     batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    model=ResUNet(b=32,nc=NUM_CLASSES,drop=0.35).to(device)
    n_p=sum(p.numel() for p in model.parameters())
    print(f"  Model  : ResUNet+CBAM+Dropout  {n_p/1e6:.2f}M params")
    ...
    if CKPT_BEST.exists():
        ...
        print(f"  Loaded : epoch {ckpt.get('epoch')} | best_dice={best:.4f}")
        print(f"  Resume : from epoch {start_epoch}\n")

    optim=torch.optim.AdamW(model.parameters(),lr=LR,weight_decay=WEIGHT_DECAY)
    sched=torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, T_max=max(EPOCHS-start_epoch+1,1), eta_min=1e-6)
    ...
    status=("TARGET ACHIEVED: Dice >= 0.90!" if md>=0.90 else
            "Excellent — close to target 0.90" if md>=0.80 else
            "Strong result" if md>=0.70 else "Good progress")
    print(f"""
  ╔═══════════════════════════════════════════════════╗
